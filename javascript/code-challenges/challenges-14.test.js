...
const screenForNames = (arr) => arr.filter(name => name.match(/^[MD][rs]s?\.\s[A-Za-z]+/));
...
const toTitleCase = (arr) => arr.map(fruit => fruit[0].toUpperCase() + fruit.slice(1));
...
const sortBy = (property, arr) => arr.sort((x, y) => typeof x[property] === 'string' ? x[property].localeCompare(y[property]) : x[property] - y[property]);
