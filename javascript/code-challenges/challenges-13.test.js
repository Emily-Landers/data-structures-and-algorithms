...
const longestString = (arr) => arr.indexOf(arr.reduce((a, b) => a.length > b.length ? a : b, 0));
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const standardizePhoneNumbers = (arr) => arr.map(element => element.match(/\d/g).join(''));
...
const allHappy = (arr) => !arr.map(element => element.includes(':)') ? true : false).includes(false);
