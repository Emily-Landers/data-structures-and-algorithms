...
const containsAnd = (arr) => {return arr.filter(word => word.includes('and'));};
...
const oddValues = (arr) => {return arr.filter(odd => odd % 2 === 1);};
...
const notInFirstArray = (forbiddenValues, arr) => {return arr.filter(wrong => !forbiddenValues.includes(wrong));};
