...
const validatePin = (pin) => pin.toString().match(/^[0-9]{4}$/g) ? true : false;
...
const validateWord = (word) => typeof word === 'string' && word.match(/^[A-Za-z]{5,10}$/) ? true : false;
...
const hasNumber = (string) => typeof string === 'string' && string.match(/[A-Za-z]+\d+/) ? true : false;
...
const validateEmail = (email) => email.match(/^\w+\.?\w+@\w+\.(net|com|org)$/g) ? true : false;
...
const validatePhoneNumber = (phoneNumber) => phoneNumber.match(/^\(\d{3}\)|^\d{3}?[ -]?\d{3}[ -]?\d{4}$/) ? true : false;
